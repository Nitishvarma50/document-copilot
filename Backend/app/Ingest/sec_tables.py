...
class _TreeParser(HTMLParser):
    ...
    def handle_endtag(self, tag: str) -> None:
        normallized = tag.lower()
        for index in range(len(self._stack) - 1, 0, -1):
            if self._stack[index].tag == normallized:
...
def extract_sec_tables(html: str) -> tuple[ExtractedTable, ...]:
    parser = _TreeParser()
    parser.feed(html)
...
def tables_to_json(tables: Sequence[ExtractedTable]) -> list[dict[str, Any]]:
    return [table.to_dict() for table in tables]


def tables_to_markdown(tables: Sequence[ExtractedTable]) -> str:
    blocks: list[str] = []
    for table in tables:
        lines: list[str] = []
        if table.title:
...
def _raw_rows(table_node: _Node) -> list[list[_RawCell]]:
    ...
    for row_node in _iter_nodes(table_node, "tr"):
        cells: list[_RawCell] = []
        for cell_node in _iter_nodes(row_node, "td"):
            cells.append(
                _RawCell(
